...
for ((i=1; i<=10; i++));
do for SV in shuff.RARE*CNV*.SV.bed;
do shuf -n 500 ./ShuffleBeds/$SV| closestBed -a hg38_PRDM14_PEAKS_sorted.ChIP.bed -b stdin -d |cut -f7 > ./Output/SV_samp500/PeakToSV/$SV.500sampled$i.TruePRDM14.dist;
done;
done
...
for ((i=1; i<=10; i++));
do for SV in ./ShuffleBeds/shuff.RARE.SV.bed;
do shuf -n 500 $SV| closestBed -a ./ShuffleBeds/shuff.hg38_PRDM14_PEAKS_sorted.ChIP.bed -b stdin -d |cut -f7 > ./Output/SV_samp500/PeakToSV/$SV.500sampled$i.ShuffPRDM14.dist;
done;
done
...
do for SV in ./ShuffleBeds/shuff.RARE*CNV.SV.bed;
do shuf -n 500 $SV| closestBed -b hg38_PRDM14_PEAKS_sorted.ChIP.bed -a stdin -d |cut -f7 > ./Output/SV_samp500/SVToPEAK/$SV.500sampled$i.TruePRDM14.dist;
done;
done
