...
#AsianPop <- read.delim("C:/Users/Rocco/Dropbox/BRL/Methylation_Struct_Mut/AsianPop.txt", stringsAsFactors=FALSE)
...
ASN_Carriers = Carriers[intersect(colnames(Carriers),AsianPop[,1])]
ASN_NonCarriers = NonCarriers[intersect(colnames(NonCarriers),AsianPop[,1])]
...
CarrierDups = SV_Subset_DUP[,intersect(colnames(ASN_Carriers),colnames(trimmed_phase3_sv_map))]
NonCarrierDups = SV_Subset_DUP[,intersect(colnames(ASN_NonCarriers),colnames(trimmed_phase3_sv_map))]
...
CarrierDels = SV_Subset_DEL[,intersect(colnames(ASN_Carriers),colnames(trimmed_phase3_sv_map))]
NonCarrierDels = SV_Subset_DEL[,intersect(colnames(ASN_NonCarriers),colnames(trimmed_phase3_sv_map))]
...
#########################################################################################################
#Compare number of deletions called per individual based on TDG allele status:
CarrierCNVs = SV_Subset_CNV[,intersect(colnames(ASN_Carriers),colnames(trimmed_phase3_sv_map))]
NonCarrierCNVs = SV_Subset_CNV[,intersect(colnames(ASN_NonCarriers),colnames(trimmed_phase3_sv_map))]
