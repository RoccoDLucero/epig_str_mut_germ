...
trimmed_phase3_sv_map = read.delim("C:/Users/Rocco/Dropbox/BRL/Methylation_Struct_Mut/VariantCalls/trimmed_phase3_sv_map.vcf", stringsAsFactors=FALSE)
...
for(popFile in popID_FilesVec){
  ...
  for(SVTYPE in SVTYPE_Vec){
    ...
    #Next Select all of the SV indices where the AF is greather than 1%
    ...
    write.table(Rare_SV_1_PCT, file = paste('C:/Users/Rocco/Dropbox/BRL/Methylation_Struct_Mut/ROI_Data/','COMMON_',popHash[[popFile]],'_',SVTYPE,".txt",sep = ''),sep = "\t",quote = F,row.names = F)
